...
This bot requires a few dependencies listed above. Once those are installed on your local environment and you have `git pull`'d for the index.py and Review.py file, run `python index.py` to activate the script. The bot will scour the TrustRadius page and create a Word document with information like the reviewer's name, position, company, Sitefinity rating, and there overall review. The Word document is formatted to contain each review on it's own page, so the more reviews that the bot scans, the longer the word document.

Currently, the bot sorts the reviews by date in descending order (most recent review first.) I 
plan on adding user control to how the lists are sorted and how many reviews are return.
